...
class CO_molecule:
    ...
    def v_P(self): # return potential energy of the bond for the vibration motion
        return self.bond.k*(self.bond.axis.mag-d)**2/2
    def v_K(self): # return kinetic energy of the vibration motion
        Vc=(self.O.v*self.O.m+self.C.v*self.C.m)/(self.O.m+self.C.m)
        return self.C.m*dot(self.C.v-Vc,self.bond.axis.norm())**2/2+\
               self.O.m*dot(self.O.v-Vc,self.bond.axis.norm())**2/2
    def r_K(self): # return kinetic energy of the rotational motion
        Vc=(self.O.v*self.O.m+self.C.v*self.C.m)/(self.O.m+self.C.m)
        VOc=(self.O.v-Vc)
        VCc=(self.C.v-Vc)
        return self.O.m*(VOc.mag**2-dot(VOc,self.bond.axis.norm())**2)/2+\
               self.C.m*(VCc.mag**2-dot(VCc,self.bond.axis.norm())**2)/2
    def com_K(self): #return kinetic energy of the translational motion of the center of mass
        Vc=(self.O.v*self.O.m+self.C.v*self.C.m)/(self.O.m+self.C.m)
        return (self.O.m+self.C.m)*Vc.mag**2/2
def collision(a1, a2):
