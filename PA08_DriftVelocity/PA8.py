...
while True:
    ...
    for i in range(N):
        p=random.random()
        if(p<=prob):
            theta_XY=random.random()*2*pi
            theta_XZ=random.random()*2*pi
            v_array[i]=np.transpose([vrms*cos(theta_XY)*cos(theta_XZ),vrms*sin(theta_XY),vrms*sin(theta_XZ)])
    ...
    if int(t/dt)%2000 == 0:
        ...
        print("tau = ",tau,", Vd = ", vv/(t/dt), ", theoretical drift velocity=",q*E*tau/m)
    vd_ball.pos = vv/(t/dt)
